    ...
    def check_params(self,
        ...
        if not repo:
            if directory and not os.path.isdir(directory):
                return self.cm.error(f'Local HF dataset is used but directory "{directory}" is not found ("{__file__}")')

            if filename:
                _filename = os.path.join(directory, filename) if directory else filename
                if not os.path.isfile(_filename):
                    return self.cm.error(f'Local HF dataset is used but file "{filename}" is not found ("{__file__}")')


        if filename and 'dataset' not in filenames:
            filenames['dataset'] = filename
        elif not filename and 'dataset' in filenames:
...
#                features['library'] = xlibrary
        ...
        if skip_cmd:
            ...
            ctx['tasks']['local']['repo'] = f'datasets/{repo}'
            ...
            if path_file_cache_list:
                ...
                if path_file_cache:
                    sub_directory = os.path.join('get-hf-dataset', xrepo)
    ...
    def run(self, 
        ...
        if filenames:
            for k in filenames:
                f = filenames[k].replace('//', os.sep)
                p = os.path.join(path_root, f)
                if not os.path.isfile(p):
                    return self.cm.error(f'Dataset sub-file "{p}" not found in "{path_root}" ("{__file__})"')
                paths[k] = p
