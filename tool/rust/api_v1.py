...
class CTool(InitCTool):
    ...
    def install(self,
        ...
        if not version_simple:
            return {
                'return': 16, 
                'error': f'custom install for "rust" can use only exact/simple versions in "{__file__}"',
        ...
        if not uarch2:
            return {
                'return': 16, 
                'error': f'custom install for "rust" could not create download URL',
            }

        filename = f'rust-{version_simple}-{uarch2}-{uos}{uext}'

        url = f'https://static.rust-lang.org/dist/{filename}'

        directory = 'content'

        path_to_tool = os.path.join(os.getcwd(), directory, 'rustc', 'bin', 'rustc' + _global['host']['vars']['file_ext_exe'])
