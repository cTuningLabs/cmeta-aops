...
class CProgram(InitCProgram):
    ...
    ############################################################
    def customize_pytorchvision(self,
                                ctx: dict,
                                desc: dict = {},
                                **misc,
    ):
        ...
        if clean:
            git_repo = _global.get('clone-git-to-cache-src-pytorchvision', {}).get('path_to_git_repo', '')
            if git_repo:
...
#                    cxx_path = os.path.join(_global['compiler-cpp']['path_bin'], 'icx-cl.exe')
        ...
        _local['target_path_exe'] = os.path.join(venv_site, 'torchvision', '__init__.py')
