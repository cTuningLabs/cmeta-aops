		 ...
		 DATA_TYPE POLYBENCH_2D(C,NI,NJ,ni,nj))
{
  ...
  FILE *dump_file = fopen("tmp-ck-output-dump.txt", "w");
  if (dump_file == NULL) {
    fprintf(stderr, "Error: could not open tmp-ck-output-dump.txt for writing\n");
    return;
  }
  for (i = 0; i < ni; i++)
    for (j = 0; j < nj; j++) {
      fprintf(dump_file, DATA_PRINTF_MODIFIER, C[i][j]);
      if ((i * ni + j) % 20 == 0) fprintf(dump_file, "\n");
    }
  fprintf(dump_file, "\n");
  fclose(dump_file);
#else
