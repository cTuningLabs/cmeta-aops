...
#if defined(__APPLE__) && __has_include(<CoreML/CoreML.h>)
#define HAVE_COREML 1
#import <CoreML/CoreML.h>
#else
#define HAVE_COREML 0
#endif
...
static const char *describe_coreml_target(void) {
#if HAVE_COREML
...
int main(int argc, char *argv[]) {
  ...
  printf("==================================================================\n");
  printf("Testing naive matmul (Objective-C++ / CoreML+Metal target) ...\n\n");
  ...
  const char *coreml_target = describe_coreml_target();
  int metal_ready = matmul_init_metal();
  const char *backend = "cpu";
  if (dtype == DTYPE_FLOAT32 && metal_ready) {
    backend = "metal";
  }
  ...
  if (dtype == DTYPE_FLOAT32) {
    ...
    for (size_t r = 0; r < repeat; ++r) {
      ...
      if (backend[0] == 'm') {
        if (!matmul_f32_metal(a, b, c, m, n, k)) {
